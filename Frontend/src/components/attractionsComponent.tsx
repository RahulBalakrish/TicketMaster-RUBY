...
const AttractionsComponent = () => {
    ...
    const currentNews = news?.attractionsData?.slice(indexOfFirstNews, indexOfLastNews);
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            if (news?.attractionsData?.length === 0) {
                ...
                <>
                    ...
                    <div className="otherNews-container">
                        {currentNews?.slice(1).map((otherNews: Events, index: number) => (
                            <AttractionsCard key={index} otherNews={otherNews} />
                        ))}

                    </div>
                    <Box className="Pagination-Box">
                        <Pagination
                            count={Math.ceil(news?.attractionsData?.length / newsPerPage)}
                            page={currentPage + 1}
